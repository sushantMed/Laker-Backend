"""
Seed drugs and prescribers (lookup tables) from a JSON file.
Safe to run on application startup, and safe to run multiple times.

Pharmacies and zip codes have their own dedicated scripts:
see seed_pharmacies.py and seed_zip_codes.py.

Behavior:
- Only records that don't already exist in the DB (matched by their natural
  key: ndc for drugs, npi for prescribers) are inserted. Records that
  already exist are skipped, not re-inserted or updated.
...
PrescriberModel's natural key column is `npi`. Adjust the `_DRUG_KEY` /
`_PRESCRIBER_KEY` constants below if your actual column names differ.
"""
